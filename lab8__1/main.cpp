...
{
    std::cout << "extractTop25NumbersUsingHeap called " << '\n';
	//Marks 50
    ...
    std::cout << "initial max heap   : " << thousandNumbers.front() << '\n';
    std::sort_heap (thousandNumbers.begin(), thousandNumbers.end());

    std::cout << "final sorted :";
    for (unsigned i=0; i<thousandNumbers.size(); i++){
        std::cout << ' ' << thousandNumbers[i] << "\n";
    }
	...
	{
		std::cout << thousandNumbers.at(index) << " , ";
		//if ((index%stepSize)+1 == 10)
			//std::cout << std::endl;
		result[j] = thousandNumbers.at(index);
		j--;
	}
	std::cout << "result\n";
...
std::vector<int> extractTop25NumbersUsingSorting(std::vector<int> thousandNumbers)
{
    ...
    std::cout << "Sorted";
    for (std::vector<int>::iterator index=thousandNumbers.end(); index!=thousandNumbers.begin(); --index)
    {
        if(stepSize!=0)
        {
            int temp = *index;
