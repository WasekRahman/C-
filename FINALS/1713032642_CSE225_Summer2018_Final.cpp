...
std::vector<int> merge(std::vector<int> &Vector, int first, int mid, int last)
{
    int index = first;
    std::queue<int> left;
    std::queue<int> right;
...
std::vector<int> merge_sort(std::vector<int> &Vector, int first, int last)
{
    ...
    if (first < last)
    {
        ...
        merge_sort(Vector, middle + 1, last);
...
std::vector<int> extractTop25NumbersUsingSorting(std::vector<int> thousandNumbers)
{
    ...
    /*******************************************/
    /*In this case, I used "MERGE SORT". At first, I was going to use Quick Sort, but it is not stable. In its worst case,
      it has a runtime of  O(n2) while in its best, it has a runtime of O(nlogn). Merge Sort has a constant time of O(nlogn)
      as it's best runtime to it's worst. The reason why I mainly decided to go with merge sort is because it is stable, and quick
      sort is not. Moreover, for such a problem, with a huge amount of input data, I believe it's easier and faster with merge sort,
      since, it splits the initial amount of data and continues to do so, making pairs and then these pairs are sorted and merged.
      Going for other sorting methods, would have meant that we would have had to traverse from the beginning to the end over and over
      again to find out the sorted values in a particular order.*/
